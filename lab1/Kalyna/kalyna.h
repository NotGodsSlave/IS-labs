...
public:
	...
	uint64_t* EncryptString(std::string plaintext, uint64_t* key, int& outLen);
	uint64_t* DecryptString(uint64_t* ciphertext, uint64_t* key, int inLen);
	void PrintState(size_t length, uint64_t* state);
	std::string BytesToString(uint8_t* bytes);
	uint64_t* StringToWords(std::string in_str, int& len);
...
private:
    ...
    void MatrixMultiply(uint8_t matrix[8][8]);
    void MixColumns();
    void InvMixColumns();
    void EncryptRound();
    void DecryptRound();
    ...
    void AddRoundKeyExpand(uint64_t* value);
    void XorRoundKey(int round);
    void XorRoundKeyExpand(uint64_t* value);
